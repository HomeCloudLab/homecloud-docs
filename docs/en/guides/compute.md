...
The machine may hold **two attached** PTY bridges at a time (Session + VS Code foreground). The VS Code panel can keep **many logical terminals**; switching away **parks** the shell on the guest (same process, resume later) instead of holding many live WebSockets. **+** adds a tab; at most two stay attached — others park (LRU). Closing a tab (**×** / trash) **kills** that shell on the guest (parked tabs reattach briefly to deliver the kill). A bare disconnect without an explicit park also terminates the shell so abandoned “Connecting…” sessions cannot fill the machine. At the parked capacity limit, the oldest parked shell is evicted. **Ctrl+J** / **Ctrl+`** or the **Terminal** toolbar button shows/hides the panel (closed by default; hiding parks attached tabs). Drag the top edge to resize.
