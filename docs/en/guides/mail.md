...
2. **Mailbox client** — folders (Inbox, Sent, Drafts, Trash, Archive), search, compose, settings. The footer bell shows **one unread row per mailbox** (count only) and opens that inbox (`/console/mail/{email}`). The client shares the tab's **one** Realtime Gateway SSE (`mail.received` soft-refreshes the list); it does not open a second `/realtime/events` stream.  
