...
| Live UI | Realtime Gateway SSE hint; REST is source of truth. The console uses **one SSE connection per tab**; Tasks listens on that stream (it does not open a second `/realtime/events`). Opening a task sets `last_seen_at` for you (WhatsApp-style). Members with `tasks.read` (and no `resources.read`) still get the live stream and the bell for Tasks — the gateway is not a Mail-only channel. |
...
Create lives only on the Tasks page. Compute, MDB, and Domain detail pages do not have a **Task** button.
...
**All**, **Mine**, and **Watching** share one workspace: Kanban columns Open / In progress / Waiting/Blocked / Done (drag cards to change status), or one table. Right-click a card for open, copy link, **Share**, status, watch, assign to me, archive, and delete. Use the grid/list icon to switch layout; each table row has the same actions under **⋯**. Tabs only change which items appear.
...
Open `TASK-12` to edit title and body, change status/assignee/due date, set labels, and continue the discussion. Details are Markdown: **Edit** shows the source (`#` headings, lists, tables); after save they render as formatted text, including `#so/…` console links, `https://` URLs, emails, and phone numbers. The title preview uses the same links. On a phone the discussion panel has a fixed height range so the thread stays readable; on a wide screen it sits beside the form. Your own bubbles use a soft wash of the console theme color (same hue, much lighter) so the text stays readable. They do not have a thick side stripe — that marker is only on quoted snippets, in the theme color. Quoted replies from others use a slightly darker fill so the original text is distinct from the reply. `@name` mentions render bold in the theme color. URLs, emails, phone numbers, and `#` console refs use the same theme color. Line breaks in discussion messages are kept (**Shift+Enter** for a new line, **Enter** to send). Hover a message (desktop) shows a smile button vertically centered on the side of the bubble; click it or right-click the message for a closed catalog of 16 HomeCloud icons — not OS emoji. Each person has one reaction per message (another pick replaces it). Click a chip to add or remove yours; hover the chip to see who reacted. The count number appears only from 2, and chips sit on the inner side of the bubble (toward the center of the thread). There is no email. Your own messages show a double-check next to the time: grey after save, blue after someone on the item (creator, assignee, or watcher — not you) has opened the task. Right-click **Details** only on **your** messages for sent time and who viewed. Right-click also copies the message text. Paperclip attaches files into that account’s bucket **`tasks-{account_number}`** under `tasks/{task_key}/chat/…` (`so://tasks-{account_number}/tasks/TASK-n/chat/…`) that lists in Object Storage, IAM, and the CLI. Images and videos show as a compact album (at most four tiles; three pictures use one tall cell like WhatsApp). Extra files overlay **+N** so a long set does not hide the composer. Video and audio play in the bubble (native controls, including fullscreen on video) after you pick them and after send — no popup. PDF uses the same inline preview as Object Storage. Pending files before send are a single-row strip you can scroll (playable types stay in a short stack). Tap an image tile to expand. Caption text sits below. **Open in Object Storage** is the full object page (20 MB per message). The discussion thread is edge to edge (no side padding). The hover smile sits vertically centered on the bubble; reaction chips sit below the timestamp on the inner side (toward the center of the thread) with a fixed grey frame. Adding a reaction does not jump the thread to the latest message. Paperclip, composer, and send share the same height. Deleted messages show a tombstone for 24 hours, then leave the thread; you cannot quote them. Link-unfurl cards are not in this release (the console does not fetch Open Graph previews). You can multi-select your own messages (long-press, then tap more) and delete them for everyone. You cannot multi-select someone else's message (delete, edit, and the rest of those actions are only on yours). Quote them from the right-click menu or by swiping the bubble inward (the avatar moves with it; the smile button hides while you drag). Swiping outward does not quote. Consecutive comments from the same person on the same day (with no system line or other speaker in between) group as a cluster; the avatar and bubble tail sit on the last message. Type `@username` to mention someone in the thread (discussion messages do not send email). Type `#` in title, details, or discussion to insert a console reference (`#mail/noreply@example.com/inbox`).

### Share (view-only links)

Anyone with the URL can open a **frozen card snapshot** without signing in (same idea as Diagram share links).
