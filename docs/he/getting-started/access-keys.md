...
1. פתחו **IAM → Users → [user] → Access keys**, או את המלאי **IAM → Access Keys**.  
2. לכל יוזר יש **מפתח פעיל אחד**. אם כבר יש מפתח, השתמשו ב-**Rotate** (מנפיק זוג חדש ומבטל מיד את כל המפתחות הפעילים הקודמים של אותו יוזר).  
3. העתיקו **פעם אחת**:

   - **Access Key ID** — מתחיל ב-`HCAK…`  
   - **Secret access key** — מוצג רק ביצירה/החלפה

הטבלה מציגה **משתמש, שם, נוצר, שימוש אחרון, סטטוס**. ה-`HCAK` לא נשאר מוצג ברשימה.

ההרשאות מגיעות מ-**IAM של היוזר**, לא מהמפתח. מפתחות עודפים בחשבונות ישנים ממשיכים לעבוד עד שמחליפים למשתמש הזה.

אם איבדתם את ה-secret, החליפו (או בטלו) את המפתח. לא ניתן לשחזר את ה-secret.
