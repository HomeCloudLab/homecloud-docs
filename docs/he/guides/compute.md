...
- **סייר** — רשימה או רשת, כמו Object Storage. תיקיית ברירת המחדל היא בית האורח (`/home/homecloud`, או `C:\Users\homecloud` ב-Windows). תיקיות נפתחות במקום. קבצים נפתחים ב-`/console/compute/{id}/file?path=` (Monaco, **שמירה ידנית** בלבד). תמונות בתצוגה מקדימה; קבצים גדולים או בינאריים להורדה.
- **VS Code** — עץ וטאבי עורך **בתוך** קבצים. פתיחת קובץ לא עוזבת את הטאב. טאבים לא שמורים מסומנים; אין שמירה אוטומטית.

כללי שחרור כמו Object Storage: שחרור על **תיקייה** מעלה לתוכה; שחרור על אזור ריק מעלה ל**תיקייה הנוכחית**; **קובץ** אינו יעד. העלאת תיקייה מסרגל הכלים או בגרירת תיקייה. תקרה **32 MiB** לקובץ.

סינון שמות חל על **הרשימה הנוכחית בלבד**. חיפוש בתוך קובץ הוא Ctrl+F של Monaco על קובץ שכבר נטען. אין grep על כל האורח.

נתיבי פלטפורמה מושמטים מהרשימה ונדחים בקריאה/כתיבה: `/etc/homecloud/**`, `/usr/local/bin/homecloud-agent`, יחידת systemd, ו-Windows `C:\ProgramData\HomeCloud\`. בית המשתמש `/home/homecloud` **לא** מוסתר.
