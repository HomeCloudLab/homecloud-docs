...
מסך מלא מכסה את **כל הדפדפן**: בלי כותרת סשן, בלי שוליים בצדדים. Esc או יציאה ממסך מלא מחזירים. החיבור נשאר כל עוד כרטיסיית הדפדפן גלויה (פינגים בפרוטוקול כל 20 שניות כדי שפרוקסי IDLE לא ינתק). עזיבת הכרטיסייה ל־**2 דקות** מנתקת ומציגה התחבר מחדש. העתקה/הדבקה: תפריט ימני או Ctrl+C / Ctrl+V / Ctrl+X (Ctrl+C מעתיק כשיש בחירה; אחרת SIGINT).
