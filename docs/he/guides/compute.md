...
המכונה מחזיקה עד **שני** גשרי PTY מחוברים בו־זמנית (סשן + טאב פעיל ב-VS Code). בפאנל VS Code אפשר להחזיק **הרבה טרמינלים לוגיים**; מעבר לטאב אחר **מציב במנוחה (park)** את ה-shell על האורח (אותו תהליך, ממשיכים אחר כך) במקום להחזיק הרבה WebSocket חיים. **+** מוסיף טאב; לכל היותר שניים מחוברים — השאר במנוחה (LRU). **Ctrl+J** / **Ctrl+`** או כפתור **טרמינל** מציגים/מסתירים את הפאנל (סגור כברירת מחדל; הסתרה מציבה במנוחה). גרירת הקצה העליון משנה גודל. סגירה (פח / × ברשימה) הורגת את ה-shell.
