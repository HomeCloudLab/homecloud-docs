...
- **VS Code** — עץ וטאבי עורך **בתוך** קבצים, ופאנל **טרמינל משולב** לצד העץ (לא מתחתיו). הכרום תואם לסביבת העבודה של **פונקציות**: ערכת נושא כהה לעורך, סרגל (**שמירה**, **עיצוב מסמך**, **פקודות**, **מסך מלא**, **הגדרות**), אייקונים בראש העץ (קובץ/תיקייה חדשים, רענון, כיווץ), breadcrumbs מתחת לטאבים, minimap והעדפות עורך משותפות עם פונקציות (דרך הגדרות). הטרמינל **סגור כברירת מחדל**; פתיחה ב-**Ctrl+J** או **Ctrl+`** (כמו ב-VS Code), בכפתור **טרמינל** בסרגל, או בבקרות הפאנל. אם Chrome/Edge עדיין פותח **הורדות** ב-Ctrl+J — השתמשו ב-Ctrl+` או בכפתור; ב-[vscode.dev](https://vscode.dev/) לרוב זה נתפס טוב יותר (PWA/workbench). כשיש **יותר מטרמינל אחד**, הרשימה מופיעה ב**סיידבר ימני** (כמו ב-VS Code שולחני), לא כטאבים למעלה. שמות הטאבים נקיים (בלי מונים כמו `bash (13)`): לפי תיקיית סביבת העבודה ומתעדכנים מכותרת החלון של ה-shell כשהאורח שולח אותה (בלי סקריפטים מוזרקים לטרמינל). ברירת המחדל היא שורש מערכת הקבצים; **פתיחת תיקייה** קובעת שורש סביבת עבודה לעץ ול-`cwd` של הטרמינל (best-effort; נשמר בדפדפן לפי מכונה). שורש הסביבה הוא **סידור IDE בלבד** — לא ארגז חול אבטחתי; סשן נשאר מעטפת מלאה. פתיחת קובץ לא עוזבת את הטאב. טאבים לא שמורים מסומנים; אין שמירה אוטומטית (אופציונלי **עיצוב בשמירה** בהגדרות). ב־**JS / TS / JSON** יש שירות שפה מובנה של Monaco בדפדפן (השלמה ואבחנות). בשפות אחרות (כולל Python) יש בעיקר צבעי תחביר והשלמה לפי מילים במסמך — אין BasedPyright / LSP על קבצי האורח (בניגוד לפונקציות).

המכונה מחזיקה עד **שני** גשרי PTY מחוברים בו־זמנית (סשן + טאב פעיל ב-VS Code). בפאנל VS Code אפשר להחזיק **הרבה טרמינלים לוגיים**; מעבר לטאב אחר **מציב במנוחה (park)** את ה-shell על האורח (אותו תהליך, ממשיכים אחר כך) במקום להחזיק הרבה WebSocket חיים. **+** מוסיף טאב; לכל היותר שניים מחוברים — השאר במנוחה (LRU). סגירת טאב (**×** / פח) **הורגת** את ה-shell על האורח (טאב במנוחה מתחבר לרגע כדי לשלוח kill). ניתוק בלי park מפורש גם מסיים את ה-shell — כדי שסשנים תקועים ב-"Connecting…" לא ימלאו את המכונה. במגבלת קיבולת, ה-shell הישן ביותר במנוחה מפונה. **Ctrl+J** / **Ctrl+`** או כפתור **טרמינל** מציגים/מסתירים את הפאנל (סגור כברירת מחדל; הסתרה מציבה במנוחה). גרירת הקצה העליון משנה גודל.
...
טאבי workspace של מכונה: **סקירה** (בריאות, גישה, resize — פעולות מחזור חיים בכותרת), **רשת** (Security groups / Floating IP / VPC כשה-placement תומך), **סשן**, **קבצים**, **ביצועים**, **Snapshots**. סשן וקבצים ממתינים לתגובת השרת. בזמן עבודה, תא **המצב** ברשימה מציג **טבעת התקדמות** קומפקטית (מילוי קו, בלי אחוזים) ותג שלב קצר (provisioning, השרת עולה, עצירה, אתחול מחדש, …). בכותרת הפרטים מוצגת אותה התקדמות **רק בזמן פעולת מחזור חיים פעילה** (או כשל/תקיעה) — בלי תג idle של פועלת/מכין ליד Start/Stop. הטבעת ברשימה נעלמת כשה-VM Running והשרת מגיב. מצב תקוע בעליית השרת מופיע אחרי 20 דקות ברשימה.
