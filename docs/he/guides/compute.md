...
טאב **סשן** לא מתחבר עד שבוחרים שיטה ולוחצים התחבר. מכונות לינוקס מציעות **מעטפת** (PTY). מכונות Windows מציגות גם **שולחן עבודה** (עדיין לא זמין). SSH `:22` נשאר break-glass ואינו שיטת סשן בקונסול.
...
- **VS Code** — עץ וטאבי עורך **בתוך** קבצים, באותו שורש. פתיחת קובץ לא עוזבת את הטאב. טאבים לא שמורים מסומנים; אין שמירה אוטומטית. בסרגל סביבת העבודה אותן פעולות **חדש**, **העלאה**, **שינוי שם**, **הורדה** ו**מחיקה** כמו בסייר (חדש/העלאה לתיקייה הממוקדת, או לתיקיית האב של הקובץ הממוקד).

כללי שחרור כמו Object Storage: שחרור על **תיקייה** מעלה לתוכה (או מעתיק קובץ שכבר בסייר); שחרור על אזור ריק מעלה ל**תיקייה הנוכחית**; **קובץ** אינו יעד. שכבת ההעלאה המלאה מופיעה רק בגרירת קבצים מהמערכת. העלאת תיקייה מתפריט **העלאה** או בגרירת תיקייה. תקרה **32 MiB** לקובץ.

סרגל: תפריט **חדש** אחד (קובץ או תיקייה) ותפריט **העלאה** אחד (קבצים או תיקייה). **שינוי שם**, **מחק** ו**הורדה** מופיעים אחרי בחירת קובץ (סייר), בעמוד הקובץ, או במיקוד/פתיחה ב-VS Code. לחיצה ימנית על קובץ או תיקייה בסייר (רשימה או רשת) או בעץ/טאבים של VS Code פותחת את הפעולות שרלוונטיות לפריט. במסך צר פעולות הסרגל הן אייקונים בלבד. שינוי שם הוא **באותה תיקייה, לקבצים בלבד** (העתקה ואז מחיקה; אין RPC של rename). אי אפשר לשנות שם של תיקייה או להוריד תיקייה כ-zip מהקונסול.

סינון שמות חל על **הרשימה הנוכחית בלבד**. חיפוש בתוך קובץ הוא Ctrl+F של Monaco על קובץ שכבר נטען. אין grep על כל האורח.

נתיבי פלטפורמה מושמטים מהרשימה ונדחים בקריאה/כתיבה: `/etc/homecloud/**`, `/home/homecloud/**`, `/usr/local/bin/homecloud-agent`, יחידת systemd, ו-Windows `C:\ProgramData\HomeCloud\` ו-`C:\Users\homecloud\`. שמות שמתחילים ב-`.` מוסתרים בסייר.
