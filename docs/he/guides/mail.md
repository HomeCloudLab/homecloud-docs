...
2. **לקוח תיבה** — תיקיות (Inbox, Sent, Drafts, Trash, Archive), חיפוש, כתיבה, הגדרות. פעמון הכותרת התחתונה מציג **שורה אחת של לא-נקראו לכל תיבה** (ספירה בלבד) ופותח את ה-inbox (`/console/mail/{email}`).  
