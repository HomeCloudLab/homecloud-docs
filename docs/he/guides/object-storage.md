...
### באקטי שירות של החשבון

הפלטפורמה עשויה ליצור buckets בבעלות החשבון עבור תכונות מוצר:

| Bucket | מטרה |
|--------|------|
| `tasks-{account_number}` | קבצי דיון במשימות |
| `diagrams-{account_number}` | תמונות nodes בדיאגרמות |
| `fn-artifacts-{account_number}` | חבילות גרסאות של פונקציות |

הם בתשלום ומופיעים תחת **הצג באקטים של שירותי החשבון**. משתמשים שאינם root יכולים לראות אותם ברשימה אך לא לפתוח, לדפדף, לערוך או למחוק. משתמש ה-**root** של החשבון יכול לפעול עליהם כמו על bucket רגיל. אל תיצרו בעצמכם buckets עם קידומות השם השמורות האלה.
...
5. **העתקה / גזירה / הדבקה** (וגרירה בין תיקיות) רצה כ**ג'וב בשרת**. העברה אטומית לכל אובייקט (העתקה + מחיקה ב-SO). סגירת הקונסול לא משאירה העתקה במקום גזירה. ההתקדמות באותו פאנל הניתן למזעור כמו בהעלאה.
