...
דף **רשימת התורים** מציג שמות מיד, ואז ממלא עומק / בטיפול / DLQ מסטטיסטיקות חיות. המספרים מתעדכנים גם כל 30 שניות וגם בלחיצה על **Refresh**, בלי למחוק את הערכים הקודמים מהמסך.
...
הודעות שנכשלו מגיעות ללשונית **DLQ** אחרי שמיצו את **מקסימום השליפות**. כל ניסיון כושל עדיין מגיע ל-worker; אם לא מוחקים לפני שנגמר visibility בניסיון האחרון, הפלטפורמה מעבירה ל-DLQ אוטומטית (בלי שליפה נוספת). בדקו מטענים, תקנו את הצרכן, ואז מחקו או נקו הודעות DLQ כשסיימתם.
...
1. הגדירו **מקסימום שליפות** (למשל `5`) על התור.  
2. אחרי חמש מסירות בלי מחיקה, כשנגמר חלון ה-visibility החמישי, ההודעה עוברת ל-DLQ.  
3. עקבו אחרי ספירת **DLQ** ב-`queues list --live`.  
4. `receive-dlq`, בדקו מטען, תקנו באג.  
5. `purge-dlq` או מחקו sequences בודדים כשנפתר.
...
- **מקסימום שליפות** מגביל מסירות ל-worker. אחרי הניסיון האחרון, המעבר ל-DLQ קורה כשנגמר visibility — לא בשליפה מאוחרת יותר.  
