...
גישה תוכנתית ל-HomeCloud מ-Python 3.

**חבילה:** `homecloud-sdk` · **ייבוא:** `from homecloud import HomeCloud`
...
אם החבילה עדיין לא ב-PyPI בסביבה שלכם:
...
`from homecloud_sdk import …` עדיין עובד לתאימות; העדיפו `from homecloud import …`.
...
קובץ אישורים: `~/.homecloud/credentials` (JSON מרובה־פרופילים).

## קשר ל-CLI

[`homecloud-cli`](https://github.com/HomeCloudLab/homecloud-cli) הוא עטיפת Typer/Rich סביב ה-SDK הזה. העדיפו את ה-CLI לפקודות סנכרון ב-shell/CI ואת ה-SDK בתוך קוד אפליקציה.
